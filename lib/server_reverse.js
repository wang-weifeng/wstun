      ...
      if (uri.query.dst != undefined){
        ...
        if(allow_list_file != undefined ){
            if(client_uuid != undefined){
                ...
                logger.info("Client " + client_uuid + " found in the allowlist as " + JSON.stringify(c));
                var p = c.filter(it => it.port === portTcp);
                logger.info("Allowed port " + JSON.stringify(p[0].port));
                
                if(JSON.stringify(p[0].port) == portTcp){
