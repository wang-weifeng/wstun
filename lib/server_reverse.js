    ...
    return function(request){
      ...
      if (uri.query.dst != undefined){
        ...
        if(allow_list_file != undefined ){
            if(client_uuid != undefined){
                var s =  allow_list.filter(it => it.client === client_uuid);
                logger.info("Client " + client_uuid + " found in the allowlist as " + JSON.stringify(s));
                var allowed_port = s.port;
                logger.info("Allowed port " + allowed_port);
                
                if(allowed_port == portTcp){
                    logger.info("Port is allowed: " + portTcp);
                    ...
                    logger.info("Port is not allowed: " + portTcp);
                    //reject the request
